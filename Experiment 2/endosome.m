...
function dydt = fullSystem(t, y, p)
    ...
    % 14. R DS
    dydt(14) = p.delta_*p.kL_*y(11) - p.delta*p.kL*L*y(14) ...
        + p.kGDs_ * y(16) - p.kGDs * y(17) * y(14); 

    % 16. R DS G
    dydt(16)= - p.kGDs_ * y(16) + p.kGDs * y(17) * y(14) - p.kGTPDs*y(16);
    % 
    % 17 G DS
    dydt(17)= p.kGDs_*y(16) - p.kGDs*y(17)*y(14) + p.c_*y(18) - p.c*y(17);
    % 
    % 18 cAMPi produced
    dydt(18)= p.c*y(17) - p.c_*y(18);
...
p.vd_ = 0.9597;
p.vd = 2.5825;
p.kGDs_ = 5.103* 10^(-6)* 60;
p.kGDs = 4.726 * 10 ^ 6 * 60;
p.kLDs_ = 0.0134* 60;
p.kLDs = 1.922 * 10^7* 60;
p.kLDs = 1.922 * 10^2* 60;
...
L_2_values = [1e-7];
g_in_values = p.GTot .* [0 1e-3 1e-2 1e-1];
...
p.L = 1e-7;
p.g = 0;
% SECOND SOLVER: varying g protein available in endosome
for g = g_in_values
    p.g = g;
    ...
    plot(t2, a .* (y2(:,18) + y2(:,1)), 'LineWidth', 3);  % plot cAMP vs time
end
...
ylim([0 40]);
ylabel('S(t) = a[cAMP]');
legend('g endo = 0','g endo = .001','g endo = .01','g endo = .1');
title('Neuron - ExD3 With Endosome');
f = gcf;
exportgraphics(f, 'exd3 endo neuron.png', 'Resolution', 300);